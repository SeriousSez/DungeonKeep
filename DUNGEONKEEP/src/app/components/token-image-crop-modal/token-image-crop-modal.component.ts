...
@Component({
    selector: 'app-token-image-crop-modal',
    standalone: true,
    imports: [CommonModule],
    templateUrl: './token-image-crop-modal.component.html',
    styleUrl: './token-image-crop-modal.component.scss',
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class TokenImageCropModalComponent {
    private static readonly CROPPER_SIZE = 320;
    private static readonly OUTPUT_SIZE = 512;
    private static readonly MIN_ZOOM = 1;
    private static readonly MAX_ZOOM = 4;

    readonly open = input(false);
    readonly imageUrl = input('');
    readonly tokenName = input('Token');
    ...
    readonly displayScale = computed(() => {
        ...
        const baseScale = Math.max(
            TokenImageCropModalComponent.CROPPER_SIZE / width,
            TokenImageCropModalComponent.CROPPER_SIZE / height
        );

        return baseScale * this.zoom();
    });
    readonly previewTransform = computed(() => {
        return `translate(-50%, -50%) translate(${this.offsetX()}px, ${this.offsetY()}px) scale(${this.displayScale()})`;
    });
    ...
    updateZoom(value: string | number): void {
        ...
        const nextZoom = Number.isFinite(numericValue)
            ? Math.max(TokenImageCropModalComponent.MIN_ZOOM, Math.min(TokenImageCropModalComponent.MAX_ZOOM, numericValue))
            : 1;
    ...
    applyCrop(): void {
        ...
        const cropWidth = TokenImageCropModalComponent.CROPPER_SIZE / scale;
        const cropHeight = TokenImageCropModalComponent.CROPPER_SIZE / scale;
        ...
        canvas.width = TokenImageCropModalComponent.OUTPUT_SIZE;
        canvas.height = TokenImageCropModalComponent.OUTPUT_SIZE;
    ...
    private clampOffsets(): void {
        ...
        const maxOffsetX = Math.max(0, (displayedWidth - TokenImageCropModalComponent.CROPPER_SIZE) / 2);
        const maxOffsetY = Math.max(0, (displayedHeight - TokenImageCropModalComponent.CROPPER_SIZE) / 2);
